...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.transcript_event_category.currentTextChanged.connect(
            self._on_transcript_event_category_changed
        )
        transcript_events.addWidget(self.transcript_event_category)
        self.transcript_event_subtype = QComboBox()
        self.transcript_event_subtype.setToolTip("Subtype for overlap annotations")
        transcript_events.addWidget(self.transcript_event_subtype)
        self.transcript_event_speaker = QComboBox()
        self.transcript_event_speaker.addItems(["P1", "P2", "P3", "P4", "Moderator", "GROUP"])
        ...
        self._on_transcript_event_category_changed(self.transcript_event_category.currentText())
    ...
    def _insert_transcript_line_after_current(self) -> None:
        ...
        subtype = self.transcript_event_subtype.currentText().strip()
        text = self.transcript_event_text.text().strip()
        ...
        type_code = type_map.get(category_lc, "SPK")

        # Canonical speaker defaults for group-level events.
        if type_code in {"OVL", "SIL", "LAU"}:
            speaker = "GROUP"

        if not text:
            if type_code == "OVL":
                overlap_sub = subtype or "simultaneous"
                text = f"[OVL:{overlap_sub}]"
            elif type_code == "SIL":
                text = "(0.3)"
            elif type_code == "BCK":
                text = "mm-hmm"
        ...
        new_row: dict[str, object] = {
            ...
            "category": category,
            "type": type_code,
            "subtype": subtype if type_code == "OVL" else "",
            "mic": "shared",
            "confidence": "manual",
            "_start": start,
            "_end": start + duration,
    ...
    def _on_transcript_event_category_changed(self, category: str) -> None:
        category_lc = category.strip().lower()
        self.transcript_event_subtype.blockSignals(True)
        self.transcript_event_subtype.clear()

        if category_lc == "overlap":
            self.transcript_event_subtype.addItems(
                ["competitive", "floor_fight", "simultaneous", "backchannel_ovl"]
            )
            self.transcript_event_subtype.setEnabled(True)
            if self.transcript_event_speaker.findText("GROUP") >= 0:
                self.transcript_event_speaker.setCurrentText("GROUP")
        else:
            self.transcript_event_subtype.addItem("")
            self.transcript_event_subtype.setEnabled(False)

        self.transcript_event_subtype.blockSignals(False)
