...
PHYSIO_FEATURES = ["group_hr_mean_bpm_mean","group_eda_phasic_rate_hz_mean","group_hrv_rmssd_ms_mean",
    "group_eda_tonic_mean_mean","group_temp_mean_mean","group_et_pupil_mean_mean"]
TRANSCRIPT_FEATURES = ["tr_silence_duration_s","tr_backchannel_count","tr_overlap_count",
...
ALL_FEATURES = [c for c in PHYSIO_FEATURES+TRANSCRIPT_FEATURES+CONTEXT_OVERLAP_FEATURES+LEXICAL_FEATURES if c in windows.columns]

df = windows.dropna(subset=["tr_spk_duration_s"]).copy() if "tr_spk_duration_s" in windows.columns else windows.copy()
keep = [c for c in ALL_FEATURES if df[c].isna().mean() <= 0.4]
imputer = SimpleImputer(strategy="mean")
X_raw = imputer.fit_transform(df[keep])
...
print(f"n_pcs={n_pcs}, n_sequences={len(sequences_X)}, total_windows={sum(len(s) for s in sequences_X)}, lengths_range=({min(len(s) for s in sequences_X)},{max(len(s) for s in sequences_X)})")

class GaussianHMM:
    def __init__(self, n_states=3, n_iter=100, tol=1e-4, random_state=42):
        self.K = n_states; self.n_iter = n_iter; self.tol = tol
    ...
    def fit(self, sequences):
        D = sequences[0].shape[1]; K = self.K
        ...
        for iteration in range(self.n_iter):
            ...
            for X in sequences:
                ...
                ll = logsumexp(log_a[-1]); total_ll += ll
                ...
                xi = np.exp(log_xi)
                pi_num += gamma[0]; A_num += xi.sum(axis=0); weight_s += gamma.sum(axis=0)
                mean_num += gamma.T @ X
                for k in range(K):
...
for k in [2, 4, 6]:
    ...
    m.fit(sequences_X)
    dt = time.time() - t0
    print(f"k={k}: {dt:.2f}s for one fit (converged, log_likelihood={m.log_likelihood_:.1f})")
